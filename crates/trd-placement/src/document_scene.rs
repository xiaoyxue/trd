...
mod tests {
    ...
    #[test]
    fn origin_basis_uses_equal_full_edge_lengths_without_offset_or_lift() {
        let frame = frame();
        let quad = quad_frame(
            CameraIntrinsics {
        ...
        let full_edge = 2.0 * quad.axis_length;
        for start in [0, 4, 8] {
            let length = columns[start..start + 3]
                ...
                .sum::<f32>()
                .sqrt();
            assert!((length - full_edge).abs() < 1e-5);
        }
        assert!((matrix.determinant() - full_edge.powi(3)).abs() < 1e-5);
    }
