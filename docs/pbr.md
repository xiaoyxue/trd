...
- [Typed PBR model](#typed-pbr-model)
  - [CPU domains](#cpu-domains)
  - [GPU uniforms](#gpu-uniforms)
  - [Preserved glTF data](#preserved-gltf-data)
- [Tone mapping](#tone-mapping)
- [HDR environment probe](#hdr-environment-probe)
  - [Reflection input](#reflection-input)
  - [Background sky](#background-sky)
- [Interactive editing — `trd-gui`](#interactive-editing--trd-gui)
...
**Rule: the CPU model mirrors shader inputs instead of folding them into one
catch-all struct.**
...
| Type | Responsibility |
|---|---|
| [`DisneyMaterial`](../crates/trd-core/src/material/disney.rs) | owns only the **11 Burley 2012 surface parameters**, plus unshaded import metadata |
| [`Lighting`](../crates/trd-core/src/light.rs) | owns ambient fill and the fixed key/fill/rim rig gain; `Light` and `PointLight` give both uniform arrays typed CPU representations |
| [`ImageBasedLighting`](../crates/trd-core/src/render/env_map.rs) | owns the per-object environment-reflection gain |
| `EnvMapData` | owns the HDR probe data |
| `Environment` | owns the HDR probe GPU binding |
| [`ToneMapping`](../crates/trd-core/src/render/tonemap.rs) | owns the per-object operator and exposure |

`Lighting` is a **scene** property — set it on the `Scene` you render
(`Scene::with_lighting`), not through a renderer setter (#182).

### GPU uniforms

**Rule: GPU PBR state is split by frequency of change (#182).**

| Uniform | Binding | Contents | Update cadence |
|---|---|---|---|
| `PbrSceneUniform` | group 0, binding 0 | camera terms + the light rig | **once per frame** |
| `PbrUniform` | group 0, binding 1 | 80-byte per-mesh slot selected with a dynamic offset | per draw |

Both share group 0 because `pbr.wgsl` already uses all four bind groups and the
portable WebGPU baseline guarantees only four. The single 304-byte uniform this
replaces re-encoded the same rig into every mesh's slot each frame.
...
**Rule: `--env <file.hdr>` binds the HDR probe that metallic surfaces reflect.**
The map is an **equirectangular HDR** environment map (Radiance `.hdr`). The
image is decoded in the shell (trd-core does no file I/O) and downscaled to the
renderer's 2048 px limit; `--env-intensity` scales the reflection (0 disables
it).

Sample maps live in [`assets/envmap/`](../assets/envmap/) (`uffizi-large.hdr`,
`cathedral.hdr`, `museum.hdr`, `ballroom.hdr`, `grace-new.hdr`).
...
**Rule: drawing the probe as the sky is a scene option, independent of object
PBR.** The probe can also be **drawn as the background sky**, camera-centered
behind every primitive:

| Surface | Control |
|---|---|
| `trd-cli` / `trd-app` | `--env-background`, blurred by `--env-background-blur`, tone-mapped with `--exposure` / `--tonemap` |
| `trd-gui` | **Environment background** toggle, with its **own** blur, exposure and tone-map operator |
| Browser renderers | `setEnvBackground(enabled, blur)` |

The sky needs a bound probe: with no `--env` the placeholder 1×1 black one is
drawn. Its **yaw is one value**, `Lighting::environment.rotation` (an
[`EnvironmentLight`](../crates/trd-core/src/light.rs)), driving the visible sky
and the reflections together.

<details>
<summary>Why it works this way</summary>

The sky's blur, exposure and tone-map operator are independent of the per-object
ones in the PBR panel (#235 S6: the sky used to borrow mesh 0's, so editing the
first object's exposure silently re-graded the background). It is a
`RenderOptions` field (`env_background`) applied by the shared
`Scene::from_draws` assembly, so every front-end gets it from the same inputs
(#235 R2) — before that only `trd-gui` could draw a sky, by reaching around the
assembly. The yaw used to exist twice, per-mesh and per-scene, with nothing
keeping them equal (#182).
</details>
