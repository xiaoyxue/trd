...
```text
L = length(r1)
P(u,v,w) = O + u*r1 + v*(L/length(r2))*r2 + w*(L/length(e3))*e3
...
and gives every axis length `length(r1)`. There is no separate half-edge or
`axis_length` height multiplier. The mesh, its AABB and
