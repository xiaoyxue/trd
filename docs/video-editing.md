...
| `unpresented tail` | samples stored but **never shown** — `none (stts)`, `N sample(s) not presented (AV_PKT_FLAG_DISCARD)`, or `unknown`. A recorder stopping mid-interval writes a final sample outside the presentation window; it is counted in `frame count` but no decoder outputs it, so the last timeline index has no picture behind it (#324). The bracketed term is the **evidence**, and it differs by surface: the browser walks the container's `stts`/`ctts` against the track duration (measured from the `elst` media time), while natively ffprobe reports ffmpeg's `AV_PKT_FLAG_DISCARD` — naming one surface's mechanism for both was misleading (#331). `unknown` means *not checked*, which is deliberately distinct from `none` — natively it is not probed for URL sources, because a second remote probe would double the cost of opening one (#326) |
